...
use crate::service_type::{
    // 以下仅做演示，实际上要根据你已有的 request/response 定义来调整
    Command,
    ConnectData,
    ConnectRequest,
    ConnectResponse, // 这里的 Request/Response 根据你 service_type.rs 的定义
                     // 也可以在此导入其它命令类型
    ConnectStatus,
};
...
impl Network {
    ...
    pub fn new<A: ToSocketAddrs>(
        ...
        let addr = format!(
            "{}:{}",
            franka_address.to_socket_addrs()?.next().unwrap().ip(),
            franka_port
        );
    ...
    fn deserialize_connect_response(&self, raw: &[u8]) -> FrankaResult<ConnectResponse> {
        ...
        let response = ConnectResponse {
            header: (), // 你可以在 service_type 里定义更具体的 Header
            status,
        };
        Ok(response)
    }
